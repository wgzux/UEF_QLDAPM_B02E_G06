...
@php
  ...
  $totalSan = $totalRaw ? preg_replace('/\\D+/', '', $totalRaw) : null;
@endphp
...
<span class="text-black dark:text-white">{{ $rooms }} Phòng</span>
<span class="text-black dark:text-white">{{ $nights }} Đêm</span>
<span class="text-black dark:text-white">{{ $adults }} Người lớn</span>
...
<span>{{ $children }} trẻ em (&lt; 6 tuổi)</span>
</div>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
<div class="sm:col-span-2">
<label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Họ và tên *</label>
<input name="name" required class="w-full border-gray-300 dark:border-gray-600 dark:bg-surface-dark focus:ring-primary focus:border-primary text-sm px-3 py-2" placeholder="Nhập họ và tên" type="text"/>
</div>
</div>
...
<div class="lg:col-span-5 space-y-10">
  <form id="confirm-form" method="GET" action="/payment">
...
<input name="phone" required class="flex-1 border-gray-300 dark:border-gray-600 dark:bg-surface-dark focus:ring-primary focus:border-primary text-sm px-3 py-2" placeholder="Nhập số điện thoại" type="tel"/>
</div>
</div>
<div>
<label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Email *</label>
<input name="email" required class="w-full border-gray-300 dark:border-gray-600 dark:bg-surface-dark focus:ring-primary focus:border-primary text-sm px-3 py-2" placeholder="Nhập địa chỉ email" type="email"/>
  <input type="hidden" name="room_type_id" value="{{ request('room_type') ?? (isset($roomType) ? $roomType->id : '') }}" />
  <input type="hidden" name="rooms" value="{{ $rooms }}" />
  <input type="hidden" name="nights" value="{{ $nights }}" />
  <input type="hidden" name="total" value="{{ $totalSan ?? '0' }}" />
  <input type="hidden" name="adults" value="{{ $adults }}" />
  <input type="hidden" name="children" value="{{ $children }}" />
  <input type="hidden" name="check_in" value="{{ $ciDt->format('Y-m-d') }}" />
  <input type="hidden" name="check_out" value="{{ $coDt->format('Y-m-d') }}" />
</div>
...
<button type="submit" form="confirm-form" class="w-full bg-primary hover:bg-[#a6764d] text-white font-medium py-3 uppercase tracking-wider text-sm transition-colors shadow-sm inline-block text-center">
